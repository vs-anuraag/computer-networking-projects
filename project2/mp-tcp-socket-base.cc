  ...
  static TypeId tid = TypeId("ns3::MpTcpSocketBase")
      ...
      .AddAttribute("CongestionControl",
          ...
          MakeEnumChecker(Uncoupled_TCPs,   "Uncoupled_TCPs",
                          ...
                          UNCOUPLED, "UNCOUPLED",
			  Semi_Coupled,    "Semi_Coupled"	
)) //Xuan: Added algorithm
...
MpTcpSocketBase::OpenCWND(uint8_t sFlowIdx, uint32_t ackedBytes)
{
  ...
  uint32_t mss = sFlow->MSS;
  double sm = 0; //Xuan added variable for summation
  Time time = sFlow->rtt->GetCurrentEstimate();
  ...
  if (cwnd < ssthresh)
  ...
  else
    {
      ...
      case Semi_Coupled:   
          ...
          tmp = (1*mss*mss)/(tt * sm);
          sFlow->cwnd = cwnd + tmp;
          //Xuan todo: CAP window increase  W= (1*mss*mss)/(rtt * summation) ,sFlow->cwnd.Get()
